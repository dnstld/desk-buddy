    ...
    try {
      ...
      if (!data?.success) {
        ...
        if (data?.error === "Company already has an owner") {
          // Invalidate queries to refresh user data and show the new owner
          ...
          showError(
            "Someone else has already taken ownership of this company. Check the Users tab to see who the owner is."
          );
      ...
      // Invalidate queries to refresh the user data and users list
      await queryClient.invalidateQueries({ queryKey: ["user"] });
      await queryClient.invalidateQueries({
        queryKey: ["users", userData.company_id],
      });
